...
class NotificacionController extends Controller
{
        ...
        if ($bandera)
        {
            ...
            if ($pedido->save() && $repartidor->save()) {
                ...
                if ($repartidor->usuario->token_notificacion) {
                    $this->enviarNotificacion($repartidor->usuario->token_notificacion, 'Se%20te%20ha%20asignado%20un%20solicitud%20S00'.$pedido_id, $pedido->id, 8,$pedido->id);
                }
    ...
    public function finalizarPedido(Request $request, $repartidor_id)
    {
        ...
        //$obj=json_encode($pedido);
        $this->enviarNotificacion($proveedor->token_notificacion, 'Se%20ha%20finalizado%20con%20éxito%20la%20solicitud%20S00'.$pedido->id, $pedido->id, 3, $obj);
